...
def home():
  ...
  SELECT 
      ...
      ROW_NUMBER() OVER(ORDER BY SUM(DISTINCT CASE WHEN u.BLOCK_TIME > CURRENT_DATE - INTERVAL '7 days' THEN ACTUALGASCOST_USD END) DESC) AS RN_7D,
      ...
      ROW_NUMBER() OVER(ORDER BY SUM(DISTINCT CASE WHEN u.BLOCK_TIME > CURRENT_DATE - INTERVAL '30 days' THEN ACTUALGASCOST_USD END) DESC) AS RN_30D,
      ...
      ROW_NUMBER() OVER(ORDER BY SUM(DISTINCT CASE WHEN u.BLOCK_TIME > CURRENT_DATE - INTERVAL '90 days' THEN ACTUALGASCOST_USD END) DESC) AS RN_90D
